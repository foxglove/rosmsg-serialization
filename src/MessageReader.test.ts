...
describe("MessageReader", () => {
  describe("simple type", () => {
    ...
    testNum("float64", 8, 0xdeadbeefcafebabe, (buffer) =>
  ...
  describe("array", () => {
    ...
    describe("typed arrays", () => {
      it("uint8[] uses the same backing buffer", () => {
        ...
        const buffer = new Uint8Array([0x03, 0x00, 0x00, 0x00, 0x01, 0x02, 0x03, 0x04]);
  ...
  describe("complex types", () => {
    ...
    it("parses bytes and constants", () => {
      ...
      const message = reader.readMessage<{
        level: boolean;
        status: { level: number; name: string };
      }>(buffer);
